...
const priceFilter = (priceRanges, items) => {
	return items.filter((i) => {
		return priceRanges.some((range) =>
			range.active ? i.price >= range.min && i.price <= range.max : false
		);
	});
};
...
const getPriceRanges = (items) => {
	...
	// Smallest step larger than 1/5 of highest price
	const step = steps.find((s) => s * 5 >= highest);
...
const filterItems = (items, searchQuery, category, priceRanges = []) => {
	...
	filteredItems = category
		? categoryFilter(category, filteredItems)
		: filteredItems;
	// Filter By Keyword
	filteredItems =
		searchQuery.length > 0
			? searchWordFilter(searchQuery, filteredItems)
			: filteredItems;
	if (priceRanges.length > 0 && priceRanges.some((range) => range.active)) {
		filteredItems =
			priceRanges.length > 0
				? priceFilter(priceRanges, filteredItems)
				: filteredItems;
	}
